...
16. **ETW Create 事件语义澄清：git 注入面 Create 降级 Audit**（2026-09-20，实机误报定案）：`FileIo/Create`（opcode 64）对应 **IRP_MJ_CREATE——任何 CreateFile 打开都触发（含纯读打开）**，经典事件字段无 CreateDisposition，无法区分读写意图。实机暴露：Git Bash（ZCode 集成终端）`__git_ps1` 提示符功能在 bash.exe 进程内读 `.git/config`（每条命令一次），读打开的 Create 事件被 on_file_create 按写语义判为注入面 Block（14:34:23 误报复录，新构建已默认不杀故仅告警噪声）。拍板：注入面的 Create 事件只出 **Audit 取证**；Block 判定由携带真实 access 的 DiskIO Read/Write 事件（on_file_open）出——Create 事件本身已把 FileObject→Name 建入缓存，紧随的真实读写必然缓存命中，**降级不损失检出**（真写 hooks/config 时 Write 事件照出 Block，git_dir_kill 门控不变）。工作流面 Create 维持 root 首见取证（防 `__git_ps1` 类高频读打开刷库）。归档产物分支不受影响：新建产物文件名即信号，且 on_file_open 无归档判定、Create 是唯一触发点（读打开已有 zip 的理论误报面：树内解压先被 tar/zip 类 cmd-block 拦截，低频可容）。m1-demo 场景断言同步按 action=block 收紧（Create 的 Audit 不计入 Block 断言）。
