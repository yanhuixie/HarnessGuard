...
- 配置/规则/SQLite/token 文件 ACL（拍板记录 14）：仅 SYSTEM/root/admin **可写**，普通用户**只读**（db 含 WAL 衍生文件 `-wal`/`-shm`）——配置、审计数据、token 均是用户需要读取的内容，仅管理员收紧导致非提权用户取用不便；写权限仍仅特权进程，防篡改配置/白名单/审计记录；
...
14. **保护文件放宽为普通用户只读（config.toml / harnessguard.db 含 WAL 衍生文件 / web-token.txt 三件套统一）**（2026-09-20，项目所有者拍板；先 token 后同日推广至全三件套）：原 §8.2 将三件套设为仅 SYSTEM/Administrators，实机使用中非提权用户读取 token（粘贴进浏览器）、查看配置与审计数据均需提权，代价过高。拍板统一改为 **SYSTEM/Administrators 全控 + BUILTIN\Users 只读**（受保护 DACL，切断继承）：这些内容本身是用户要读取的东西；写仍仅特权进程（config/db 防篡改，token 每次服务启动重写即轮换）。db 的 `-wal`/`-shm` 衍生文件一并纳入（Users 可读 db 而不可读 WAL 会导致读库内容不一致/失败）。自检口径同步修订：允许型 ACE 受托者为 SYSTEM/Administrators（至少其一在场）或 **Users 仅读**——Users ACE 含任一写位（FILE_WRITE_DATA/APPEND/WRITE_EA/DELETE_CHILD/WRITE_ATTRIBUTES/DELETE/WRITE_DAC/WRITE_OWNER）仍判未保护并自愈收紧。已知 tradeoff：本机任意进程（含被攻陷的 harness）可读 token、审计数据与配置——接受该面，理由：Web UI 仅绑定 127.0.0.1 且有 Host 校验，被攻陷进程在本机已具备读文件同等能力的其他路径，读权限不是决定性增量；写面（篡改配置/白名单/审计证据）仍封闭。install 与服务启动自检均按新口径应用，旧版仅管理员收紧过的文件升级/重启后自动覆盖为可读态。
15. **计划任务持久化事件仅在归因到监控树时告警，无 harness 关联一律抑制**（2026-09-20，项目所有者拍板）：Windows 系统计划任务（`\Microsoft\` 命名空间等）由系统服务自注册/自更新（TaskScheduler 操作日志 106/140/141，事件常无 pid/UserContext），ETW 源为全系统订阅，原「ProcTable 在表即采、树外系统进程注册任务也是事实证据」语义把这类高频常态噪声全量写入审计流并逐条触发通知（实机实例：`\Microsoft\Windows\Flighting\OneSettings\RefreshCache` 更新事件未归因告警，与任何 harness 无关）。拍板：计划任务事件仅在存在 harness 关联时出 `RawEvent::Persistence`——① 事件 pid 在身份表且属监控树（事实归因）；② 树内注册工具进程实时扫描；③ Exec 时缓存的注册工具 cmdline 候选（启发式，"候选无法限树 + detail 附 cmdline 供人工核对"的既有披露边界不变）。树外进程注册与全链未命中（未归因）一律不 emit，仅 `tracing::debug!` 留痕。依据：需求 §3.5 范围本为「**harness 注册**计划任务」，本次系实现对齐需求而非放宽；树外持久化不在本产品威胁模型内。代价披露：① 树外注册不再产生审计证据（含用户手工 schtasks 等合法操作）；② 事件 pid 在表但属树外时直接抑制、不落启发式链——宁可缺归因不产假证据（防树外注册被误挂到树内候选）。RunKey 轮询（pid=0 无归因）维持现状未同步收紧：快照对比仅新增键触发、低频，与计划任务的自更新噪声形态不同。
16. **ETW Create 事件语义澄清与注入面 Create 分档**（2026-09-20 定案，同日修订——首版"一律降 Audit"引入回归）：`FileIo/Create`（opcode 64）对应 **IRP_MJ_CREATE——任何 CreateFile 打开都触发（含纯读打开）**，经典事件字段无 CreateDisposition，无法区分读写意图。实机暴露两个事实：① Git Bash（ZCode 集成终端）`__git_ps1` 提示符在 bash.exe 进程内读 `.git/config`（每条命令一次），读打开的 Create 按写判为注入面 Block（14:34:23 误报复录）；② **`FileIo/Read`(67)/`Write`(68) 事件需 `EVENT_TRACE_FLAG_DISK_FILE_IO` 订阅，而当前仅启用 FILE_IO(_INIT)——该事件流实际不存在**（首版降级时"Block 由 Read/Write 事件兜底、降级不损失检出"的前提因此不成立，部署后注入面 Block 零触发，dashboard 阻断计数恒 0——判定列表中的大量 block 系部署前旧实例的历史记录；计数器与装配链经核查无恙）。**修订拍板：Create 事件按注入面子档分治**——`hooks/**` 的注入面本就是"读写皆拦"，Create 无需读写意图即出 Block（Create 回归为注入面唯一可靠触发点，git_dir_kill 门控不变）；`config`/`config.lock` 注入面仅对写生效，Create 证明不了写意图（读打开同事件形态）只出 Audit——config 真写的增强检测留待 DISK_FILE_IO 订阅评估（其 67/68 事件按文档语义也仅覆盖真实磁盘 IO，缓存命中不发，不能作为唯一触发点）。工作流面 Create 维持 root 首见取证（防 `__git_ps1` 类高频读打开刷库）。归档产物分支不受影响：新建产物文件名即信号，Create 是唯一触发点（读打开已有 zip 的理论误报面：树内解压先被 tar/zip 类 cmd-block 拦截，低频可容）。m1-demo 场景断言同步按 action=block 收紧。
